...
enum EXPR_TYPE {
    BIN_EXPR, UNARY_EXPR, ID_EXPR, NUM_EXPR, STRING_EXPR, FUNC_EXPR, LIST_EXPR, SUBSCRIPT_EXPR, LAMBDA_EXPR, BLESS_EXPR, ARRAY_CON_EXPR, TYPE_EXPR, FIELD_EXPR
};

enum STMT_TYPE {  
    PROGRAM_STMT, IF_STMT, WHILE_STMT, PRINT_STMT, DEF_STMT, EXPR_STMT, BLOCK_STMT, STRUCT_STMT, LET_STMT, RETURN_STMT, STMT_LIST, RECORD_DEF_STMT
};
...
void preorder(AST* ast, int d) {
    if (ast != nullptr) {
        ...
        cout<<ast->token.getString()<<", "<<(ast->token.scopeLevel() == -1 ? "":to_string(ast->token.scopeLevel()))<<endl;
